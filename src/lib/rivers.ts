...
export type RiverConfig = {
  macroChunkSpan: number;
  sourceSkipChance: number;
  searchRays: number;
  searchStep: number;
...
export const DEFAULT_RIVER_CONFIG: RiverConfig = {
  macroChunkSpan: 8,
  sourceSkipChance: 0.35,
  searchRays: 16,
  searchStep: 128,
  maxSearchDistance: 4096,
  pointSpacing: 64,
  bendAmplitude: 24,
  intersectionMargin: 24,
  collisionWidth: 12,
};
...
function riverPath(seed: number, source: WorldPoint, config: RiverConfig): WorldPoint[] {
  ...
  for (let angleIndex = 0; angleIndex < config.searchRays; angleIndex += 1) {
    ...
    for (
      let distance = config.searchStep;
      distance <= config.maxSearchDistance;
      distance += config.searchStep
    ) {
      const candidate = {
  ...
  const length = Math.max(
    8,
    Math.ceil(Math.hypot(target.x - source.x, target.y - source.y) / config.pointSpacing),
  );
  ...
  for (let index = 0; index <= length; index += 1) {
    ...
    const bend =
      Math.sin(t * Math.PI * 3 + seed * 0.03) * config.bendAmplitude * Math.sin(t * Math.PI);
...
export function riversForChunk(
  coord: ChunkCoord,
  seed: number,
  chunkSize: number,
  ...
  for (let y = macroY - 1; y <= macroY + 1; y += 1)
    for (let x = macroX - 1; x <= macroX + 1; x += 1) {
      ...
      const source = {
        x:
          x * chunkSize * config.macroChunkSpan +
          120 +
          seededHash(seed, x, y, 8101) * chunkSize * 6,
        y:
          y * chunkSize * config.macroChunkSpan +
          120 +
          seededHash(seed, x, y, 8102) * chunkSize * 6,
      };
      if (biomeAt(source, seed, terrainConfig) !== 'mountains') continue;
      const id = `river:${x},${y}`;
      const path = riverPath(seed, source, config);
      const relevant = path.some(
        (point) =>
          point.x >= origin.x - config.intersectionMargin &&
          point.x <= origin.x + chunkSize + config.intersectionMargin &&
          point.y >= origin.y - config.intersectionMargin &&
          point.y <= origin.y + chunkSize + config.intersectionMargin,
      );
