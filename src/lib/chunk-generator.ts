...
export class ProceduralChunkGenerator implements ChunkGenerator {
  ...
  constructor(config: Partial<WorldConfig> = {}) {
    ...
    this.settlementGraph = new SettlementGraph(this.config.seed, this.config.terrain);
    this.treeGenerator = new ProceduralTreeGenerator(this.config);
  }
  generate(coord: ChunkCoord): WorldChunk {
    const key = chunkKey(coord),
      origin = chunkOrigin(coord, this.config.chunkSize);
    const center = {
      x: origin.x + this.config.chunkSize / 2,
      y: origin.y + this.config.chunkSize / 2,
    };
