...
export class WorldManager implements WorldService {
  ...
  getLoadedChunks(): ReadonlyArray<WorldChunk> { return [...this.chunks.values()]; }
  syncAround(player: WorldPoint): ReadonlyArray<WorldChunk> {
    ...
    for (let y = -this.config.tier2Radius; y <= this.config.tier2Radius; y += 1) for (let x = -this.config.tier2Radius; x <= this.config.tier2Radius; x += 1) {
      const coord = { x: this.center.x + x, y: this.center.y + y }, key = chunkKey(coord);
      next.add(key);
    ...
    for (const [key, chunk] of this.chunks) {
      ...
      else if (next.has(key)) this.chunks.set(key, { ...chunk, tier: tierForDistance(distance, this.config) });
