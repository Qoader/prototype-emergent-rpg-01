...
function walkable(cell: Cell, world: WalkabilityMap): boolean {
  const center = cellCenter(cell);
  return (
    world.isWalkable(center) &&
    world.isWalkable({ x: center.x + 7, y: center.y + 7 }) &&
    world.isWalkable({ x: center.x - 7, y: center.y - 7 })
  );
}
...
function findPathToCandidates(
  ...
  const open = new Set<string>([key(start)]),
    ...
    estimate = new Map<string, number>([[key(start), distance(start, candidates[0])]]),
  ...
  while (open.size > 0 && expanded.size < maxExpansions) {
    ...
    for (const candidate of open)
      if ((estimate.get(candidate) ?? Infinity) < (estimate.get(currentKey) ?? Infinity))
    ...
    for (const direction of directions) {
      ...
      if (nextCost < (cost.get(nextKey) ?? Infinity)) {
        ...
        estimate.set(nextKey, nextCost + distance(next, candidates[0]));
        open.add(nextKey);
      }
    }
  }
