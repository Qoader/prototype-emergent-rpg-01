...
export class SettlementRoadNetwork {
  ...
  constructor(seed: number, terrain: TerrainConfig) {
  ...
  roadsForChunk(
    ...
    const origin = chunkOrigin(coord, chunkSize),
      sources = universe.filter(
        (node) =>
          Math.abs(node.center.x - regionCenter.x) < 4096 &&
          Math.abs(node.center.y - regionCenter.y) < 4096,
      ),
      roads: Road[] = [];
    for (const edge of this.topology.buildEdges(sources, universe)) {
      ...
      if (
        road.points.some(
          (point) =>
            point.x >= origin.x - 160 &&
            point.x <= origin.x + chunkSize + 160 &&
            point.y >= origin.y - 160 &&
            point.y <= origin.y + chunkSize + 160,
        )
      )
        roads.push(road);
    }
