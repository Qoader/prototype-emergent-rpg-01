...
const distance = (a: Settlement, b: Settlement) =>
...
export class SettlementTopologyService {
  ...
  buildEdges(
    ...
    const edges: SettlementEdge[] = [],
      ids = new Set<string>();
    const connect = (from: Settlement, to: Settlement, type: RoadType | undefined) => {
      if (!type) return;
    ...
    for (const node of sources) {
      const parentType: SettlementType | undefined =
        node.type === 'city'
          ? 'capital'
          : node.type === 'village'
            ? 'city'
      ...
      if (parents.length)
        connect(
          node,
          parents.reduce((nearest, item) =>
            distance(node, item) < distance(node, nearest) ? item : nearest,
          ),
          ROAD_FOR[node.type],
        );
