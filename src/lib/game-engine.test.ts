...
function fakeWorld(): WorldService {
  const chunk = { key: '0,0', coord: { x: 0, y: 0 }, tier: 0, biome: 'plains', trees: [], houses: [], rivers: [], lastSimulatedAt: 0 } as WorldChunk;
  return { syncAround: () => [chunk], getChunkAt: () => chunk, getLoadedChunks: () => [chunk], isWalkable: () => true, tick: () => {} };
}

describe('GameEngine', () => {
  it('moves headlessly and keeps its renderer-independent snapshot', () => {
    const engine = new GameEngine(fakeWorld(), new AStarNavigator(), { movementSpeed: 16 }, { now: () => 1000 });
