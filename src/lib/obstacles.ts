...
export class WorldCollisionService implements CollisionService {
  isBlocked(point: WorldPoint, chunk: CollisionSource, config: Partial<WorldConfig> & Pick<WorldConfig, 'seed'>): boolean {
  ...
  for (const feature of chunk.layout ?? []) if (feature.blocked) {
    if ((feature.kind === 'wall' || feature.kind === 'fence') && feature.points) {
      for (let index = 1; index < feature.points.length; index += 1) if (distanceToSegment(point, feature.points[index - 1], feature.points[index]) <= (feature.width ?? 4) / 2) return true;
    } else if (pointInRect(point, { x: feature.bounds.minX, y: feature.bounds.minY, width: feature.bounds.maxX - feature.bounds.minX, height: feature.bounds.maxY - feature.bounds.minY })) return true;
  }
...
export function isPointBlocked(point: WorldPoint, chunk: CollisionSource, config: Partial<WorldConfig> & Pick<WorldConfig, 'seed'>): boolean { return defaultCollisionService.isBlocked(point, chunk, config); }
