...
export class PixiRenderer implements RendererAdapter {
  ...
  constructor(private readonly worldService: WorldService, private readonly onTarget: (point: { x: number; y: number }) => void) {}
  async mount(host: HTMLElement): Promise<void> {
    this.host = host;
    await this.app.init({ background: 0x090d1d, antialias: true, resizeTo: host, resolution: Math.min(devicePixelRatio, 2) });
    host.appendChild(this.app.canvas); this.app.stage.addChild(this.world); this.world.addChild(this.camera); this.camera.addChild(this.map); this.map.addChild(this.playerRenderer.view);
    this.world.eventMode = 'static'; this.world.on('pointertap', (event) => { const point = event.getLocalPosition(this.world); this.onTarget(screenToWorld(point, this.camera.position, this.zoom)); });
    this.app.renderer.on('resize', this.resize, this); this.resize();
  }
  ...
  private resize(): void { this.zoom = Math.min(1.35, Math.max(0.85, this.app.screen.width / 620)); this.world.hitArea = new Rectangle(0, 0, this.app.screen.width, this.app.screen.height); }
}
